...
public class OVXFlowMod extends OFFlowMod implements Devirtualizable {
	...
	private void prepAndSendSouth(OVXPort inPort, boolean pflag) {
		...
		try {
			psw = OVXMap.getInstance().getPhysicalSwitches(sw).get(0);
		} catch (SwitchMappingException e) {
			log.error(e.toString());
		}

		String query = "INSERT INTO FlowMod_Entries VALUES('"
				+ Integer
						.toHexString(U16.f(this.getMatch().getDataLayerType()))
				+ "','"
				...
				+ "','"
				+ cidrToString(this.getMatch().getNetworkDestination(), this
						.getMatch().getNetworkDestinationMaskLen())
				+ "','"
				...
				+ HexString.toHexString(this.getMatch()
						.getDataLayerDestination()) + "','"
				+ HexString.toHexString(this.getMatch().getDataLayerSource())
				+ "','" + this.getActions().toString() + "','"
				+ System.currentTimeMillis() + "','" + sw.getSwitchName()
				+ "','" + sw.getTenantId() + "','" + psw.getSwitchName()
				+ "');";
		DBHandler.insertValues(query);

		// log.info("**** Match **** {} \n **** Action **** {}",
		// this.match.toString(), this.getActions().toString());
		// Mappable m=OVXMap.getInstance();
		// PhysicalIPAddress pip=new
		// PhysicalIPAddress(this.getMatch().getNetworkDestination());
		/*
		 * try {
		 * log.info("**** Virtual IP {}****",m.getVirtualIP(pip).toString()); }
		 * catch (AddressMappingException e1) { // TODO Auto-generated catch
